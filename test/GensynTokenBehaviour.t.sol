...
abstract contract GensynTokenBehavior is Tester {
    // ===== ERRORS =====
    ...
    function test_InitialState() external view {
        ...
        assertTrue(gensynTokenProxy.hasRole(DEFAULT_ADMIN_ROLE, address(timelock)), "unexpected default admin role");
    }

    function testFuzz_Transfer(address to, uint256 amount) external {
        uint256 gensynBalanceBefore = gensynTokenProxy.balanceOf(gensyn);

        // Bound
        vm.assume(to != address(0));
        vm.assume(to != gensyn);
        amount = bound(amount, 1, gensynBalanceBefore);

        // Validate before
        vm.assume(gensynTokenProxy.balanceOf(to) == 0);
        ...
        // Validate after
        assertEq(gensynTokenProxy.balanceOf(gensyn), gensynBalanceBefore - amount);
        assertEq(gensynTokenProxy.balanceOf(to), amount);
    }

    function testFuzz_TransferFrom(address spender, uint256 approved, uint256 value) external {
        // Validate transferrer
        vm.assume(spender != address(0));
        vm.assume(spender != gensyn);

        // Validate
        vm.assume(gensynTokenProxy.balanceOf(spender) == 0);

        // Recipient approves transferrer
        _useNewSender(gensyn);
        uint256 gensynBalanceBefore = gensynTokenProxy.balanceOf(gensyn);
        approved = bound(approved, 1, gensynBalanceBefore);
        gensynTokenProxy.approve(spender, approved);

        // Transferrer cannot transfer more than approved
        _useNewSender(spender);
        value = bound(value, approved + 1, type(uint256).max);
        vm.expectRevert(
            abi.encodeWithSelector(IERC20Errors.ERC20InsufficientAllowance.selector, spender, approved, value)
        );
        assertFalse(gensynTokenProxy.transferFrom(gensyn, spender, value));

        // Transferrer transfers gensyn's tokens
        value = bound(value, 1, approved);
        assertTrue(gensynTokenProxy.transferFrom(gensyn, spender, value));

        // Validate
        assertEq(gensynTokenProxy.balanceOf(gensyn), gensynBalanceBefore - value);
        assertEq(gensynTokenProxy.balanceOf(spender), value);
    }

    // ===== ERC20BurnableUpgradeable =====

    function _beforeBurnTest() internal virtual {
        // Validate
    ...
    function testFuzz_Burn(uint256 burn) external {
        ...
        uint256 balanceBefore = gensynTokenProxy.balanceOf(gensyn);
        uint256 supplyBefore = gensynTokenProxy.totalSupply();

        // Recipient cannot burn more than balance
        _useNewSender(gensyn);
        burn = bound(burn, balanceBefore + 1, type(uint256).max);
        vm.expectRevert(abi.encodeWithSelector(ERC20InsufficientBalance.selector, gensyn, balanceBefore, burn));
        gensynTokenProxy.burn(burn);

        // Recipient burns
        burn = bound(burn, 1, balanceBefore);
        gensynTokenProxy.burn(burn);

        // Validate
        assertEq(gensynTokenProxy.balanceOf(gensyn), balanceBefore - burn);
        assertEq(gensynTokenProxy.totalSupply(), supplyBefore - burn);
    }

    function _beforeBurnFromTest() internal virtual {
        // Validate
        assertEq(gensynTokenProxy.balanceOf(gensyn), initialSupply);
        assertEq(gensynTokenProxy.totalSupply(), initialSupply);
    }
    ...
    function testFuzz_BurnFrom(address burner, uint256 approved, uint256 burn) external {
        ...
        _useNewSender(gensyn);
        approved = bound(approved, 1, balanceBefore);
    ...
    function testFuzz_DelegateSelf(address user, uint256 amount, address delegatee) external {
        // Bound
        vm.assume(user != address(0));
        vm.assume(user != gensyn);
        amount = bound(amount, 1, gensynTokenProxy.balanceOf(gensyn) / 2);
        vm.assume(delegatee != address(0));
        vm.assume(delegatee != user);
        vm.assume(gensynTokenProxy.balanceOf(user) == 0);

        // Recipient transfers tokens to user
        _useNewSender(gensyn);
        assertTrue(gensynTokenProxy.transfer(user, amount), "recipient should be able to transfer tokens");
        ...
        assertEq(gensynTokenProxy.getVotes(user), amount, "user's votes should be equal to his balance");

        // Ensure user is his own delegate
        assertEq(gensynTokenProxy.delegates(user), user, "user's delegate should be self");
        ...
        assertEq(gensynTokenProxy.getVotes(user), 2 * amount, "user's votes should be 2 * amount");
        ...
        assertEq(gensynTokenProxy.getVotes(user), 0, "user's votes should be 0");
        assertEq(gensynTokenProxy.getVotes(delegatee), 2 * amount, "delegate's votes should be 2 * amount");
    }
    ...
    function testFuzz_DelegateBySig(
        ...
        uint256 delegatorNonce = gensynTokenProxy.nonces(delegator);
    ...
    function testFuzz_Permit(
        ...
        uint256 ownerNonce = gensynTokenProxy.nonces(owner);

        // Recipient gives owner some tokens
        _useNewSender(gensyn);
        ownerBalance = bound(ownerBalance, 1, gensynTokenProxy.balanceOf(gensyn));
        ...
        assertEq(gensynTokenProxy.allowance(owner, spender), 0, "allowance should be 0");
        assertEq(gensynTokenProxy.nonces(owner), ownerNonce, "nonces should be unchanged");

        // Execute permit
        gensynTokenProxy.permit(owner, spender, value, deadline, v, r, s);

        // Validate permit after
        assertEq(gensynTokenProxy.allowance(owner, spender), value, "allowance should be value");
        assertEq(gensynTokenProxy.nonces(owner), ownerNonce + 1, "nonces should be incremented");

        // Ensure spender can now spend the owner's tokens
        _useNewSender(spender);
        assertTrue(
            gensynTokenProxy.transferFrom(owner, spender, value), "spender should be able to spend owner's tokens"
        );
        assertEq(gensynTokenProxy.balanceOf(spender), value, "spender should have value tokens");
    }
    ...
    function testFuzz_PermitAndDelegateBySigShareNonce(
        ...
        uint256 callerNonce = gensynTokenProxy.nonces(caller);
    ...
    function testFuzz_GrantRole(address executor) external {
        ...
        _useNewSender(gensyn);
        vm.expectRevert(abi.encodeWithSelector(AccessControlUnauthorizedAccount.selector, gensyn, DEFAULT_ADMIN_ROLE));

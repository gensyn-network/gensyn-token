...
contract LayerZero_EthereumMainnet_Test is LayerZero_EthereumMainnet_Utils, Test {
    ...
    function _validateBefore() internal view {
        ...
        assertFalse(
            BRIDGED_GENSYN_TOKEN.hasRole(
                BRIDGED_GENSYN_TOKEN.MINTER_ROLE(), BRIDGED_GENSYN_TOKEN_MINT_BURN_OFT_ADAPTER
            ),
            "_validateBefore: adapter has MINTER_ROLE on the token"
        );
        assertFalse(
            BRIDGED_GENSYN_TOKEN.hasRole(
                BRIDGED_GENSYN_TOKEN.BURNER_ROLE(), BRIDGED_GENSYN_TOKEN_MINT_BURN_OFT_ADAPTER
            ),
            "_validateBefore: adapter has BURNER_ROLE on the token"
        );
    ...
    function _validateAfter(address adapterTimelock) internal view {
        ...
        assertTrue(
            ...
            "_validateAfter: adapter does not have MINTER_ROLE on the token"
        );
        assertTrue(
            BRIDGED_GENSYN_TOKEN.hasRole(
                BRIDGED_GENSYN_TOKEN.BURNER_ROLE(), BRIDGED_GENSYN_TOKEN_MINT_BURN_OFT_ADAPTER
            ),
            "_validateAfter: adapter does not have BURNER_ROLE on the token"
        );
